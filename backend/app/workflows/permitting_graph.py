...
from app.models.document import CandidateSiteRecord, Document, DocumentChunk
...
def analyze_document(
    ...
    *,
    ...
    if (
        ...
        and document.processing_status != DocumentWorkflowStatus.FAILED.value
    ):
        return build_analysis_response(db, document.id)

    if request.status == DocumentWorkflowStatus.FAILED.value:
        request.retry_count += 1

    request.status = DocumentWorkflowStatus.ANALYZING.value
    ...
    initial_state: AnalysisState = {
        ...
        "status": DocumentWorkflowStatus.ANALYZING.value,
        "validation_errors": [],
    }

    try:
        final_state = graph.invoke(initial_state)
        final_status = DocumentWorkflowStatus(final_state["status"])
        ...
        document = get_document_with_text(db, document_id)
        update_document_status(db, document, DocumentWorkflowStatus.FAILED, error_message=message)
        request.status = DocumentWorkflowStatus.FAILED.value
...
def _validate_document(db: Session) -> NodeFunction:
    def node(state: AnalysisState) -> dict[str, Any]:
        ...
        site = db.scalar(
            select(CandidateSiteRecord)
            .where(CandidateSiteRecord.id == document.site_id)
            .options(selectinload(CandidateSiteRecord.project))
        )
        if site is None:
            raise NotFoundError("Site was not found.")
        update_document_status(db, document, DocumentWorkflowStatus.VALIDATING)
        return {
            "project_id": document.project_id,
            "site_id": document.site_id,
            "project_type": site.project.project_type,
            "page_count": document.page_count,
            "status": DocumentWorkflowStatus.VALIDATING.value,
        }
...
def _extract_text(db: Session, settings: Settings) -> NodeFunction:
    def node(state: AnalysisState) -> dict[str, Any]:
        document = get_document_with_text(db, state["document_id"])
        update_document_status(db, document, DocumentWorkflowStatus.EXTRACTING)
        ...
        return {
            "page_count": document.page_count,
            "status": DocumentWorkflowStatus.EXTRACTING.value,
...
def _retrieve_sections(db: Session) -> NodeFunction:
    def node(state: AnalysisState) -> dict[str, Any]:
        document = get_document_with_text(db, state["document_id"])
        update_document_status(db, document, DocumentWorkflowStatus.RETRIEVING)
        chunks = list(
            db.scalars(
        ...
        return {
            ...
            "status": DocumentWorkflowStatus.RETRIEVING.value,
        }
...
def _flag_ambiguity() -> NodeFunction:
    def node(state: AnalysisState) -> dict[str, Any]:
        ...
        validation_errors = list(state.get("validation_errors", []))
        workflow_status = DocumentWorkflowStatus.NEEDS_REVIEW
        ...
        for item in state.get("valid_requirements", []):
            ...
            if decision.workflow_status == DocumentWorkflowStatus.PARTIALLY_COMPLETED:
                workflow_status = DocumentWorkflowStatus.PARTIALLY_COMPLETED
        ...
        if validation_errors and workflow_status != DocumentWorkflowStatus.PARTIALLY_COMPLETED:
            workflow_status = DocumentWorkflowStatus.PARTIALLY_COMPLETED
        return {
...
def _persist_findings(db: Session) -> NodeFunction:
    def node(state: AnalysisState) -> dict[str, Any]:
        ...
        status = DocumentWorkflowStatus(state["status"])
        if not persistable and status != DocumentWorkflowStatus.FAILED:
            status = DocumentWorkflowStatus.PARTIALLY_COMPLETED
        update_document_status(
            db,
            document,
            status,
...
def _get_or_create_request(
    ...
    *,
    ...
    request = DocumentAnalysisRequest(
        document_id=document.id,
        idempotency_key=key,
        status=DocumentWorkflowStatus.UPLOADED.value,
    )
    db.add(request)
    db.commit()
    db.refresh(request)
...
def _is_terminal(status: str) -> bool:
    return status in {
        DocumentWorkflowStatus.NEEDS_REVIEW.value,
        DocumentWorkflowStatus.COMPLETED.value,
        DocumentWorkflowStatus.PARTIALLY_COMPLETED.value,
    }
